export function initImageManipulation() {
  ...
  const mirrorTypeSelect = document.getElementById(
    "mirror-type"
  ) as HTMLSelectElement;
  const applyMirrorBtn = document.getElementById(
    "apply-mirror-btn"
  ) as HTMLButtonElement;
  ...
  function renderEditCanvas() {
    ...
    if (cropRect.width > 0 && cropRect.height > 0) {
      ...
      ctx.fillRect(
        ...
        editCanvas.width,
        editCanvas.height - (cropRect.top + cropRect.height)
      );
      // Left
      ...
      ctx.fillRect(
        ...
        cropRect.top,
        editCanvas.width - (cropRect.left + cropRect.width),
        cropRect.height
      );
    }
  }
  ...
  window.addEventListener("mousemove", (e) => {
    if (!isCropping || !editCanvas) return;
    const rect = editCanvas.getBoundingClientRect();

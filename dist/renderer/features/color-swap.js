...
exports.initColorSwap = initColorSwap;
function initColorSwap() {
    const dropZone = document.getElementById("swap-drop-zone");
    const fileInput = document.getElementById("swap-file-input");
    ...
    const colorAHex = document.getElementById("swap-color-a-hex");
    const colorBSwatch = document.getElementById("swap-color-b-swatch");
    const colorBHex = document.getElementById("swap-color-b-hex");
    const toleranceSlider = document.getElementById("swap-tolerance-slider");
    const toleranceValue = document.getElementById("swap-tolerance-value");
    const swapBtn = document.getElementById("swap-btn");
    const downloadBtn = document.getElementById("swap-download-btn");
    const resetBtn = document.getElementById("swap-reset-btn");
    if (!dropZone || !fileInput || !canvas || !swapBtn)
        return;
    const resultPreview = document.getElementById("swap-result-preview");
    ...
    let pickingMode = "A"; // Which color we are currently picking
    ...
    canvas.addEventListener("click", (e) => {
        ...
        if (pickingMode === "A") {
            colorA = rgb;
            if (colorASwatch)
                colorASwatch.style.backgroundColor = hex;
            if (colorAHex)
                colorAHex.value = hex.toUpperCase();
        }
        else {
            colorB = rgb;
            if (colorBSwatch)
                colorBSwatch.style.backgroundColor = hex;
            if (colorBHex)
                colorBHex.value = hex.toUpperCase();
        }
    });
        ...
        setTimeout(() => {
            try {
                ...
                for (let i = 0; i < data.length; i += 4) {
                    ...
                    const b = data[i + 2];
                    // Distance to Color A
                    const diffA = Math.sqrt(Math.pow(r - colorA.r, 2) +
                        Math.pow(g - colorA.g, 2) +
                        Math.pow(b - colorA.b, 2));
                    // Distance to Color B
                    const diffB = Math.sqrt(Math.pow(r - colorB.r, 2) +
    ...
    downloadBtn.addEventListener("click", async () => {
        if (!resultPreview.src)
            return;
        const savePath = await window.electronAPI.selectSavePath("swapped_image.png");
        if (savePath) {
    ...
    function resetState() {
        ...
        if (colorASwatch)
            colorASwatch.style.backgroundColor = "#000000";
        if (colorAHex)
            colorAHex.value = "#000000";
        colorA = { r: 0, g: 0, b: 0 };
        // Reset B to white default
        if (colorBSwatch)
            colorBSwatch.style.backgroundColor = "#FFFFFF";
        if (colorBHex)
            colorBHex.value = "#FFFFFF";

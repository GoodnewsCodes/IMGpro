...
function initVideoToGifSubTab() {
    ...
    const video = document.getElementById("v2g-video-element");
    const videoInfoBadge = document.getElementById("video-info-badge");
    const startTimeInput = document.getElementById("v2g-start-time");
    const endTimeInput = document.getElementById("v2g-end-time");
    ...
    const fpsSelect = document.getElementById("v2g-fps-select");
    const widthSelect = document.getElementById("v2g-width-select");
    const customWidthGroup = document.getElementById("v2g-custom-width-group");
    const customWidthInput = document.getElementById("v2g-custom-width");
    const qualitySelect = document.getElementById("v2g-quality-select");
    const loopCheckbox = document.getElementById("v2g-loop-checkbox");
    ...
    convertBtn.addEventListener("click", async () => {
        if (!currentVideoFile || isConverting)
            return;
        ...
        video.muted = true;
        video.pause();
        const gifEncoder = new gif_encoder_1.GifEncoder(targetWidth, targetHeight, loopCount);
    ...
    // Download GIF logic
    downloadBtn.addEventListener("click", async () => {
        if (!generatedGifBlob || !currentVideoFile)
            return;
